...
In this second dry practical, we’ll extend those skills by working with **experimental data** that you generated in the wet lab. Specifically, we’ll use your results from the **subcellular fractionation practical**, where you separated treated and untreated diseased cells into mitochondrial and cytosolic fractions, then measured protein concentration and enzyme activity.

This practical will focus on:

-   Calculating **specific enzyme activity** for each sample
-   Comparing the mitochondrial and cytosolic fractions from treated and untreated diseased cells
...
# your data may also have a lot of missing data, in which case we can clean this up by omitting all of the NA's
...
<<<<<<< HEAD
data <- read_csv("~/git/BIOL340_DP/pages/DP2/simplified_BSA_cleaned.csv")
...
data <- data %>%
  na.omit()
=======
data <- read_csv("~/git/BIOL340_DP/pages/DP2/simplified_BSA.csv")
>>>>>>> refs/remotes/origin/main
```
...
```{r}
data <- data %>%
  mutate(`rate of change` = as.numeric(`rate of change`),
         `concentration change(ΔC/s)` = `rate of change` / 13600)
```
...
6.  Now to get to specific activity we need to tie this amount value back to the protein concentration in our samples. This we calculated using a standard curve as `mg/mL`. So to get to amount in `mg` we need to times by the volume added, which in our case was 50 uL or 0.05 mL. This column has been prepared for you in the dataset. 
...
```{r}
data <- data %>%
  mutate(specific_activity = `amount TNB/min(nM)` / `protein amount`)
```
